...
class RNN(object):
    ...
    def acc_deltas_bptt(self, x, d, y, s, steps):
        ...
        for t in arange(len(d))[::-1]:
            ...
            for tao in arange(max(0, t-steps), t+1)[::-1]:
                ...
                one_hotX = make_onehot(x[tao],self.vocab_size)
    ...
    def compute_loss(self, x, d):
        ...
        Y,s = self.predict(x)
        loss = -sum([make_onehot(d[t],self.vocab_size)*log(Y[t]) for t in xrange(0,len(Y))])
        # another possibility is that they want us to compute the loss per x[item] separately ?!
    ...
    def compute_mean_loss(self, X, D):
        ...
        mean_loss = 0.
        # total = sum(map(len,D))
        # loss = sum([self.compute_loss(X[item],D[item]) for item in xrange(0,len(X))])
        # mean_loss = loss/float(total)
        mean_loss = -mean(sum([self.compute_loss(X[item],D[item]) for item in xrange(0,len(X))]))
...
if __name__ == "__main__":
    ...
    if mode == "train":
        ...
        r.train(X, D, X_dev, D_dev, 25, 0.5, 3, 10, 100, 0.0001)
