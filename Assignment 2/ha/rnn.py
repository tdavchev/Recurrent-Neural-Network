...
class RNN(object):
    ...
    def compute_mean_loss(self, X, D):
        ...
        mean_loss = 0.
        total = sum(map(len,D))
        for item in xrange(0,len(X)):
            loss = self.compute_loss(X[item],D[item])
    ...
    def generate_sequence(self, start, end, maxLength):
        ...
        print end
        print "^^^^^ END ^^^^"
        # while word != end or len(x) < maxLength:
        print len(sequence)
        while len(sequence)<maxLength and newWord!=end:
            Y,s = self.predict(x)
            newWord = multinomial_sample(Y)
            x.append(newWord) # this can change
